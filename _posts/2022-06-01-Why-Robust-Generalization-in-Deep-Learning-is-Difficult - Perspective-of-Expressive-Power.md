...
1. 주어진 데이터 $\mathcal{D}$ 가 $N$ 개의 sample 과 $d$ 의 차원을 갖는 다면, $\tilde{\mathcal{O}}(Nd)$ 의 weight 로도 100% robust training accuracy 를 달성할 수 있다.

2. 그에 반해, 이진 분류 과제에서 ReLU 네트워크로 robust test error 를 얻기 위해선 $d$ 에 대하여 exponential weight size $\exp(\Sigma(d))$ 가 요구된다. 이는 $\mathcal{D}$ 의 class 가 linear separable 하더라도 유지된다.

3. $\mathcal{D}$ 가 $k$-dimensional manifold $(k\ll d)$ 에 놓여 있다면 $\exp\mathcal{O}(k)$ 가 요구된다.
